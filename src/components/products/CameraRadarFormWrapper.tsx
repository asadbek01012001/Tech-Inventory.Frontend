...
import CameraRadarForm from "./CameraRadarForm";
...
export default function CameraradarFormWrapper({ initialValues, setInitialValues, title }: Props) {
  ...
  const onChangeProduct = useCallback(
    (index: number, type: "add" | "delete") => {
      const oldCamera = [...initialValues?.speedCheckingCamera];
      ...
      setInitialValues((prev: any) =>
        update(prev, {
          speedCheckingCamera: oldCamera,
        }),
      );
    },
    [initialValues?.speedCheckingCamera],
  );

  return (
    <ProductFormGroup title={title} addClick={() => onChangeProduct(0, "add")}>
      {initialValues?.speedCheckingCamera &&
        initialValues?.speedCheckingCamera?.map((_: any, index: number) => {
          return (
            <ProductFormItem
              key={index}
              index={index}

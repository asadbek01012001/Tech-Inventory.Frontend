...
import CameraVariofakalForm from "./CameraVariofakalForm";
...
export default function CameraVariofakalFormWrapper({
  initialValues,
  setInitialValues,
  title,
}: Props) {
  ...
  const onChangeProduct = useCallback(
    (index: number, type: "add" | "delete") => {
      const oldCamera = [...initialValues?.variofakalCamera];
      ...
      setInitialValues((prev: any) =>
        update(prev, {
          variofakalCamera: oldCamera,
        }),
      );
    },
    [initialValues?.variofakalCamera],
  );

  return (
    <ProductFormGroup title={title} addClick={() => onChangeProduct(0, "add")}>
      {initialValues?.variofakalCamera &&
        initialValues?.variofakalCamera?.map((_: any, index: number) => {
          return (
            <ProductFormItem
              key={index}
              index={index}

...
export default function CameraFormWrapper({ initialValues, setInitialValues, title }: Props) {
  ...
  const onChangeProduct = useCallback(
    (index: number, type: "add" | "delete") => {
      const oldCamera = [...initialValues?.camera];
  ...
  return (
    <ProductFormGroup title={title} addClick={() => onChangeProduct(0, "add")}>
      {initialValues?.camera &&
        initialValues?.camera?.map((_: any, index: number) => {
          return (
            <ProductFormItem
              key={index}
              index={index}

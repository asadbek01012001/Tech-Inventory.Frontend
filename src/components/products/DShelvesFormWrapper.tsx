...
import DShelvesForm from "./DShelvesForm";
...
export default function DShelvesFormWrapper({ initialValues, setInitialValues, title }: Props) {
  ...
  const onChangeProduct = useCallback(
    (index: number, type: "add" | "delete") => {
      const oldProduct = [...initialValues?.distributionShelf];
      ...
      setInitialValues((prev: any) =>
        update(prev, {
          distributionShelf: oldProduct,
        }),
      );
    },
    [initialValues?.distributionShelf],
  );

  return (
    <ProductFormGroup title={title} addClick={() => onChangeProduct(0, "add")}>
      {initialValues?.distributionShelf &&
        initialValues?.distributionShelf?.map((item: any, index: number) => {
          return (
            <ProductFormItem
              key={index}
              index={index}

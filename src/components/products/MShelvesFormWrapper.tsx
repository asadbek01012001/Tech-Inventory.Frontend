...
import MShelvesForm from "./MShelvesForm";
...
export default function MShelvesFormWrapper({ initialValues, setInitialValues, title }: Props) {
  ...
  const onChangeProduct = useCallback(
    (index: number, type: "add" | "delete") => {
      const oldProduct = [...initialValues?.mainTelecomunicationShelf];
      ...
      setInitialValues((prev: any) =>
        update(prev, {
          mainTelecomunicationShelf: oldProduct,
        }),
      );
    },
    [initialValues?.mainTelecomunicationShelf],
  );

  return (
    <ProductFormGroup title={title} addClick={() => onChangeProduct(0, "add")}>
      {initialValues?.mainTelecomunicationShelf &&
        initialValues?.mainTelecomunicationShelf?.map((item: any, index: number) => {
          return (
            <ProductFormItem
              key={index}
              index={index}
